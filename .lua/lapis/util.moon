...
-- LAPISBEAN PATCH: no json.null value (or rather, it's nil)
-- so we can't use it as a table key
encodable_userdata = {
--  [json.null]: true
}

-- LAPISBEAN NOTE: technically this is an array and not userdata
-- (EncodeJson takes an array {[0]=false} to mean empty array)
-- but there's no harm in leaving it in
if json.empty_array
